...
the normalized public products it publishes. A FireCOP source must have an
explicit public-distribution flag, an allowlist of output formats, a review
date, and a review basis before the public layers process will emit it. Missing
metadata removes the affected product from discovery and blocks direct
artifact output.

Customer enclave federation is intentionally deferred. Private products are
operator-configured inside FireCOP and use its existing authenticated session
and signed-package controls.
